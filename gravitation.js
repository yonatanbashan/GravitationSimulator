...
function getRadius() {
  ...
  let radius = getRadiusFromMass(mass);
  if (radius < 4) {
    radius = 4;
  }
  return radius;
...
function appendStar(svg, xEnd, yEnd, serial) {
  endTime = new Date();
  mousePressed = false;
...
updatePressed = function(x, y) {
  const prevTempCircle = newTempCircle;
  if (prevTempCircle) {
    prevTempCircle.remove();
    newTempCircle = undefined;
  }
  ...
  if (mousePressed == true) {
    ...
    const radius = getRadius();
    const contour = radius * 0.3;
    const contourColor = getColorFromMass(mass);
...
svg.on("mousedown", function() {
  ...
  startTime = new Date();
  window.setInterval(() => updatePressed(xStart, yStart), updateInterval);
});
...
svg.on("mouseup", function(d) {
  ...
  serial += 1;
  appendStar(svg, xEnd, yEnd, serial);
});
...
function getColorFromMass(mass) {
  ...
  Rcolor = Math.round(Rcolor * 255);
  let Bcolor = Math.floor((256 - Rcolor) / 2);

  Gcolor = componentToHex(Math.floor((Bcolor * 1) / 6 + (Rcolor * 1) / 3));
...
function waitAnimation() {
  ...
  if (speed_size > trailSpeed) {
    speed_size = trailSpeed;
  }
...
function updateLocation() {
  ...
  stars.forEach(function(element) {
    ...
    var dist = Math.sqrt(
      Math.pow(activeX - star_x, 2) + Math.pow(activeY - star_y, 2)
    );

    speed_x *= loss;
    speed_y *= loss;
    if (serial != activeSerial && dist > 1.0) {
      // Avoid applying force from same star, or from very very close star
